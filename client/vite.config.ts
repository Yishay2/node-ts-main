...
export default defineConfig(({ mode }) => ({
  ...
  preview: { port: 3001, strictPort: true, open: false }, // When running `npm run preview` locally, no loadEnvVariables process occurs, so VITE_PORT would be undefined. On a CI however, there are always ENV VARIABLES set in the global scope so it *would* work.
  css: {
    modules: {
